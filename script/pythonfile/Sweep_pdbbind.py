...
sweep_configuration = {
    ...
    'parameters':
    {
        ...
        'protein_readout': {'values': ["mean"]},
        'drug_readout': {'values': ["sum"]}, 
        'protein_concat': {'values': [False]},
        'protein_short': {'values': [False]}, # True
        'protein_hidden_dim': {'values': [256]},
        'protein_layer': {'values': [4]},
        'drug_concat': {'values': [False]},
        'drug_short': {'values': [False]},
        'drug_hidden_dim': {'values': [128]}, 
        'drug_layer': {'values': [3]},
...
sweep_id = wandb.sweep(sweep=sweep_configuration, project='PDBbind_Sweep') 
# sweep_id = ""
...
def main():
    ...
    dataset = datasets.PDBBind(protein_method="gearnetesm_pocket", drug_method="distanceMol",
                               transform=protein_view)
    ...
    # ============================== Model Defining  ============================== #
    protein_model = models.GearNet(input_dim=1280, hidden_dims=[wandb.config.protein_hidden_dim]*wandb.config.protein_layer,
    ...
    drug_model = models.RGCN(input_dim=67, hidden_dims=[wandb.config.drug_hidden_dim]*wandb.config.drug_layer, num_relation=4,
    ...
    # ============================== Normal Lr setting ============================== #
    optimizer = torch.optim.AdamW(params=task.parameters(), lr=wandb.config.learning_rate,
    ...
    checkpoint = "../../result/model_pth/pdbbind_sweep.pth"

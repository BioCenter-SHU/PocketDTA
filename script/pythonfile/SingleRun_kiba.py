...
protein_model = models.GearNet(
    input_dim=1280, hidden_dims=[128, 128, 128], num_relation=7, batch_norm=True, readout="sum"
)

# drug_model = models.GIN(
#     input_dim=67, hidden_dims=[32, 32, 32, 32, 32], batch_norm=True, readout="sum"
# )

drug_model = models.RGCN(input_dim=67, hidden_dims=[256, 256, 256, 256, 256], num_relation=4, edge_input_dim=19,
                         batch_norm=True, concat_hidden=True, readout="sum")
# ============================== task define and other training prepare ============================== #
task = tasks.InteractionPrediction(
    model=protein_model, model2=drug_model, task=dataset.tasks, mode_type="MolFormer",  #  KIBA
    criterion="mse", metric=("mse", "c_index", "rm2"), normalization=True, num_mlp_layer=3, hidden_dim=128
)

task.apply(initialize_weights)

# ============================== Normal Lr setting ============================== #
# wandb.init(project="KIBA_Experiments")
learning_rate = 1e-3
weight_decay = 0
optimizer = torch.optim.AdamW(params=task.parameters(), lr=learning_rate, weight_decay=weight_decay)

solver = core.Engine(task, train_set, valid_set, test_set, optimizer, None, gpus=[0], batch_size=256)  # , logger="wandb"

scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer, mode='min', factor=0.2, patience=6, min_lr=1e-7)
...
checkpoint = "../../result/model_pth/gearnet_k_0820.pth"

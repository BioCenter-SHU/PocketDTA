...
sweep_configuration = {
    ...
    'parameters':
    {
        ...
        'weight_decay': {'values': [1e-4]},  # 1e-4, 5e-4, 2e-3
        'protein_readout': {'values': ["mean"]},  # , "sum"
        'drug_readout': {'values': ["mean"]},
        'protein_concat': {'values': [False]},  # , True
        'protein_short': {'values': [False]},
        'protein_hidden_dim': {'values': [128, 256, 512]},
        'protein_layer': {'values': [2, 3, 4, 5]},
        'drug_concat': {'values': [False]},
        'drug_short': {'values': [False]},
        'drug_hidden_dim': {'values': [128, 300]},  # 64, 128, 512
        'drug_layer': {'values': [2, 3, 4, 5]},
        'random_seed': {'values': [42]},  # 0, 42, 3407
...
sweep_id = wandb.sweep(sweep=sweep_configuration, project='KIBA_Sweep') 
# sweep_id = "marinehdk/KIBA_Sweep/bwlp06m7"
...
def main():
    ...
    dataset = datasets.KIBA(protein_method="gearnetesm_pocket", drug_method="distanceMol", transform=protein_view)
    ...
    protein_model = models.GearNet(
        input_dim=21, hidden_dims=[wandb.config.protein_hidden_dim]*wandb.config.protein_layer,
    ...
    drug_model = models.RGCN(
        input_dim=67, hidden_dims=[wandb.config.drug_hidden_dim]*wandb.config.drug_layer, num_relation=4,
        edge_input_dim=19, batch_norm=True, short_cut=wandb.config.drug_short,
        concat_hidden=wandb.config.drug_concat, readout=wandb.config.drug_readout
    )
                             
    # ============================== Task Defining ============================== #
    task = tasks.InteractionPrediction(
        model=protein_model, model2=drug_model, task=dataset.tasks, criterion="mse",  # mode_type="DeepFusion",
        metric=("mse", "c_index", "rm2"), num_mlp_layer=3  # hidden_dim=256
    )
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer, mode='min', factor=0.2, patience=6, min_lr=1e-5)
    ...
    checkpoint = "../../result/model_pth/pocket_d_sweep.pth"

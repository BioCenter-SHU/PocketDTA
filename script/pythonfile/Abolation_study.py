...
sweep_id = wandb.sweep(sweep=sweep_configuration, project='KIBA_Sweep')  # Filtered_  Davis
# sweep_id = "marinehdk/Filtered_Davis_Sweep/b3bbr495"
...
def main():
    ...
    dataset = datasets.KIBA(protein_method="gearnetesm650m", drug_method="distanceMol",
                            transform=protein_view)  # description='filter',
    ...
    task = tasks.InteractionPrediction(
        model=protein_model, model2=drug_model, task=dataset.tasks, criterion="mse",
        metric=("mse", "c_index", "rm2"), mode_type='MolFormer'
    )  # mode_type='MolFormer',
    ...
    solver = core.Engine(
        task, train_set, valid_set, test_set, optimizer, None, gpus=[0], batch_size=64, logger="wandb"
    )
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer=optimizer, mode='min', factor=0.2, patience=6, min_lr=1e-7
    )
